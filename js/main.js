...
(function (window) {
    ...
    function openItem(item) {
        ...
        var contentItem = item.getAttribute('data-content');
        //        console.log(item);
        console.log(contentItem);
        //ORDER ITEM
        $(".color-tool").css("opacity", "0");
        $(".color-clear").css("opacity", "0");

        if (contentItem === "content-2") {
            $("#chart").hide();
            $(".color-tool").css("opacity", "1");
            $(".color-clear").css("opacity", "1");
        }

        if (contentItem === "content-1") {
            $("#chart").hide();
        }

        if(contentItem === "content-4"){
            window.isStartPlay = true;
        }

        if (contentItem === "content-3") {
            $("#chart").show();
            //            setTimeout(function () {
            //                $(".number").eq(0).addClass("bounceIn");
            //                $(".number").one('webkitAnimationEnd oanimationend msAnimationEnd animationend',function(){
            //                    $(this).removeClass("bounceIn");
            //                });
            //            }, 2000);
            setInterval(function () {
                var index = Math.random() * ($(".number").length - 0) + 0;
                //console.log(parseInt(index));
                var _text = parseInt($(".number").eq(parseInt(index)).text()) + 1;
                $(".number").eq(parseInt(index)).text(_text);
                $(".number").eq(parseInt(index)).addClass("bounceIn");
                $(".number").one('webkitAnimationEnd oanimationend msAnimationEnd animationend', function () {
                    $(this).removeClass("bounceIn");
                });
            }, 3000);
